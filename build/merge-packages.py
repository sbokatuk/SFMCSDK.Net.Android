...
SDK's Android workload ships reference packs for only the current target framework and the
previous one - the .NET 9 band covers net8/net9, the .NET 10 band covers net9/net10. The packages
are therefore built in two passes (see BuildNugets.sh) and merged here into one package per id.
...
The dependency group is copied rather than synthesised as an empty one. Every package here
declares dependencies - on its sibling Datadog packages, and on the Kotlin/AndroidX/OkHttp
binding packages that supply the Java types its .aar links against. An empty group would tell
NuGet a net10.0-android consumer needs none of them, and the app would fail at runtime with
NoClassDefFoundError the first time the SDK touched one.
...
def add_dependency_groups(nuspec: str, groups: list[str]) -> str:
    ...
    # No <dependencies> element at all - true for a package with nothing to depend
